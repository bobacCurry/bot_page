  ...
  <div>
    <Card>
      <Row type="flex" justify="start" align="middle" class="table-option">
        <Col>
          <Card class="option-card"><Button type="info" @click="createShow()">添加屏蔽词</Button></Card>
        </Col>
        ...
        <Col span="9" class="search-input-box">
          <Input
            ...
            <Select
                    ...
                    v-model="search.type"
                    @on-clear="searchKeywords(false)">
              <Option value="word">屏蔽词 (模糊匹配)</Option>
            </Select>
          </Input>
        </Col>
      </Row>
      <Table ref="filter" class="table" :loading="loading" :data="tableData" :columns="tableColumns" border>
    ...
    methods: {
      ...
      async mockTableData() {
        ...
        await this.getList({ model,page, size, conditions }).then(res => {
          this.total = res.filter_count
          data = res.filter_list
        }).catch((e)=>{
      ...
      editShow(index) {
        ...
        this.modalOpt.name = this.tableData[index].word
        this.modalOpt.flag = true
      },
      remove(index) {
        this.$Modal.confirm({
          title: `删除 ${this.tableData[index].word}`,
          onOk:()=>{
